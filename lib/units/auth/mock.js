...
module.exports = function(options) {
  ...
  app.post('/auth/api/v1/mock1', function(req, res) {
    var log = logger.createLogger('auth-mock')
    log.setLocalIdentifier(req.ip)
    switch (req.accepts(['json'])) {
      case 'json':
        urlutil.islogin(req.body.name,req.body.password,function(canLogin){
          log.info(canLogin)
          requtil.validate(req, function() {
              req.checkBody('name').notEmpty()
          ...
          .then(function() {
            log.info('Authenticated "%s"', req.body.name)
            log.info('Authenticated "%s"', req.body.name + "@zybk.com")
            var token = jwtutil.encode({
              payload: {
                name: req.body.name
              , email: req.body.name + "@zybk.com"
              }
             , secret: options.secret
             , header: {
                exp: Date.now() + 24 * 3600
